...
class _CheckOrderEditState extends State<CheckOrderEdit> {
  ...
  Future<void> _initializeData() async {
    ...
    print('DEBUG: Full order data for pricing: ${widget.orderData}');
    print('DEBUG: Room price per hour from order: ${widget.orderData['room_price_per_hour']}');

    _isRoomOnly = widget.orderData['is_room_only'] == 1 || widget.orderData['is_room_only'] == true;
    ...
    if (widget.orderData['meeting_room_id'] != null) {
      _selectedRoom = {
        ...
        'capacity': int.tryParse(widget.orderData['number_of_people']?.toString() ?? '0') ?? 0,
        'price_per_hour': int.tryParse(widget.orderData['room_price_per_hour']?.toString() ?? '0') ?? 0,  // Use room_price_per_hour field
      };
      print('DEBUG: Selected room initialized with price: ${_selectedRoom['price_per_hour']}');
    }
  ...
  bool _validateOrder() {
    ...
    if (!_isRoomOnly && _selectedPackages.isEmpty) {
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  !_isRoomOnly ? 'Booking dengan Paket' : 'Booking Ruangan Saja',
            ...
            if (_isRoomOnly && _selectedRoom != null) ...[
              Divider(height: 24),
              Container(
  ...
  int _calculateRoomCost() {
    ...
    final pricePerHour = _parsePrice(_selectedRoom['price_per_hour'] ?? 0);
    final ceilingHours = calculateCeilingHours(_duration);
    print('DEBUG: Calculating room cost:');
    print('- Price per hour: $pricePerHour');
    print('- Duration (minutes): $_duration');
    print('- Ceiling hours: $ceilingHours');
    print('- Total cost: ${pricePerHour * ceilingHours}');
    return pricePerHour * ceilingHours;
  }
                    ...
                    onTap: () {
                      setState(() {
                        _selectedRoom = {
                          ...
                          'price_per_hour': room['price_per_hour'] ?? widget.orderData['room_price_per_hour'], // Try to preserve original price
                        };
